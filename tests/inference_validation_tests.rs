...
use spreadsheet_mcp::sparql::{
    InferenceRule, InferenceRuleValidator, ReasoningConfig, ReasoningGuard,
    RuleDependencyAnalyzer, InferredTripleValidator, MaterializationManager,
    MaterializationConfig, MaterializationStrategy, InvalidationStrategy,
    Triple, ValidationError, PredicateBlacklist, TripleConstraint,
};
...
fn create_problematic_rule(rule_type: &str) -> InferenceRule {
    match rule_type {
        ...
        "non_monotonic" => InferenceRule {
            ...
            construct_query: "CONSTRUCT { ?x mcp:active true } MINUS { ?x mcp:archived true }".to_string(),
...
fn test_reasoning_guard_iteration_limit() {
    ...
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), ValidationError::IterationLimitExceeded { .. }));
}
...
fn test_stratification() {
    ...
    let rules = vec![
        ...
        create_valid_rule("high_level", vec!["derived1".to_string(), "derived2".to_string()]),
    ];
    ...
    let base_stratum_idx = strata.iter()
        .position(|s| s.contains(&"base1".to_string()))
        .unwrap();
    let high_stratum_idx = strata.iter()
...
fn test_justification_chain() {
    ...
    validator.record_provenance(inferred2.clone(), "rule2".to_string(), vec![base.clone(), inferred1.clone()]);
