...
impl JiraClient {
    pub fn new(base_url: String, auth_token: String) -> Result<Self> {
        ...
        Ok(Self { base_url, auth_token, client })
    }
    ...
    pub async fn search_issues(&self, jql: &str, fields: &[String]) -> Result<Vec<JiraIssue>> {
        ...
        let resp = self.client.post(&url)
            ...
            .json(&body).send().await.context("search request failed")?;
    ...
    pub async fn get_issue(&self, key: &str, fields: &[String]) -> Result<JiraIssue> {
        let url = format!("{}/rest/api/3/issue/{}?fields={}", self.base_url, key, fields.join(","));
        let resp = self.client.get(&url)
            .header("Authorization", format!("Bearer {}", self.auth_token))
            .send().await.context("get issue failed")?;
        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            bail!("Jira API error ({}): {}", status, body);
    ...
    pub async fn update_issue(&self, key: &str, fields: &JiraFieldUpdate) -> Result<()> {
        ...
        let resp = self.client.put(&url)
            .header("Authorization", format!("Bearer {}", self.auth_token))
            .header("Content-Type", "application/json")
            .json(&body).send().await.context("update issue failed")?;
        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            bail!("Jira API error ({}): {}", status, body);
    ...
    pub async fn create_issue(&self, project_key: &str, fields: &JiraFieldUpdate) -> Result<String> {
        ...
        let resp = self.client.post(&url)
            .header("Authorization", format!("Bearer {}", self.auth_token))
            .header("Content-Type", "application/json")
            .json(&body).send().await.context("create issue failed")?;
        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            bail!("Jira API error ({}): {}", status, body);
...
pub struct JiraSearchResult { pub issues: Vec<JiraIssue> }

#[derive(Debug, Clone, Deserialize)]
pub struct JiraIssue { pub key: String, pub fields: serde_json::Value }

#[derive(Debug, Clone, Deserialize)]
pub struct JiraCreateResult { pub key: String }
...
impl JiraFieldUpdate {
    fn to_json(&self) -> serde_json::Value {
        ...
        if let Some(ref s) = self.summary { fields.insert("summary".to_string(), serde_json::json!(s)); }
        if let Some(ref s) = self.status { fields.insert("status".to_string(), serde_json::json!({"name": s})); }
        if let Some(ref a) = self.assignee { fields.insert("assignee".to_string(), serde_json::json!({"name": a})); }
        if let Some(ref d) = self.description { fields.insert("description".to_string(), serde_json::json!(d)); }
        if let Some(ref p) = self.priority { fields.insert("priority".to_string(), serde_json::json!({"name": p})); }
        if let Some(ref l) = self.labels { fields.insert("labels".to_string(), serde_json::json!(l)); }
        for (k, v) in &self.custom_fields { fields.insert(k.clone(), v.clone()); }
...
pub struct JiraSyncColumnMapping {
    ...
    #[serde(default)] pub status_column: Option<String>,
    #[serde(default)] pub assignee_column: Option<String>,
    #[serde(default)] pub updated_column: Option<String>,
    #[serde(default)] pub description_column: Option<String>,
    #[serde(default)] pub priority_column: Option<String>,
    #[serde(default)] pub labels_column: Option<String>,
}
...
impl SyncReport {
    fn new() -> Self {
        Self { created: 0, updated: 0, skipped: 0, errors: Vec::new(), conflicts: Vec::new() }
    }
}
...
pub struct SyncJiraToSpreadsheetParams {
    ...
    #[serde(default)] pub column_mapping: JiraSyncColumnMapping,
    #[serde(default = "default_start_row")] pub start_row: usize,
    #[serde(default)] pub conflict_resolution: ConflictResolution,
}

fn default_start_row() -> usize { 2 }
...
pub async fn sync_jira_to_spreadsheet(
    ...
    let registry = state.fork_registry().ok_or_else(|| anyhow!("fork registry not available"))?;
    let fork_ctx = registry.get_fork(&params.fork_id)?;
    let work_path = fork_ctx.work_path.clone();

    let jira_client = JiraClient::new(params.jira_base_url.clone(), params.jira_auth_token.clone())?;

    let mut fields = vec!["summary".to_string(), "updated".to_string()];
    if params.column_mapping.status_column.is_some() { fields.push("status".to_string()); }
    if params.column_mapping.assignee_column.is_some() { fields.push("assignee".to_string()); }
    if params.column_mapping.description_column.is_some() { fields.push("description".to_string()); }
    if params.column_mapping.priority_column.is_some() { fields.push("priority".to_string()); }
    if params.column_mapping.labels_column.is_some() { fields.push("labels".to_string()); }

    let jira_issues = jira_client.search_issues(&params.jql_query, &fields).await.context("query Jira failed")?;
    ...
    for issue in jira_issues {
        ...
        if let Some(&row_num) = key_to_row.get(&jira_key) {
                    ...
                    match params.conflict_resolution {
                        ConflictResolution::JiraWins => {
                            report.conflicts.push(ConflictReport {
                                row: row_num, jira_key: jira_key.clone(),
                                reason: format!("Spreadsheet newer ({}) vs Jira ({})", sheet_ts, jira_ts),
            ...
            edits.extend(build_row_edits(&params.sheet_name, row_num, &params.column_mapping, &issue));
            report.updated += 1;
        } else {
            let new_row = params.start_row + existing_rows.len() + report.created;
            edits.extend(build_row_edits(&params.sheet_name, new_row, &params.column_mapping, &issue));
            report.created += 1;
        }
    }
    ...
    if !edits.is_empty() {
        ...
        registry.with_fork_mut(&params.fork_id, |ctx| { ctx.edits.extend(edits_for_fork); Ok(()) })?;
    }

    let fork_workbook_id = WorkbookId(params.fork_id.clone());
    let _ = state.close_workbook(&fork_workbook_id);

    Ok(SyncJiraToSpreadsheetResponse { fork_id: params.fork_id, report })
}
...
pub struct SyncSpreadsheetToJiraParams {
    ...
    #[serde(default)] pub column_mapping: JiraSyncColumnMapping,
    #[serde(default = "default_start_row")] pub start_row: usize,
    #[serde(default)] pub end_row: Option<usize>,
    #[serde(default)] pub conflict_resolution: ConflictResolution,
}
...
pub async fn sync_spreadsheet_to_jira(
    ...
    for (row_idx, row) in spreadsheet_rows.iter().enumerate() {
        ...
        if row.summary.is_none() || row.summary.as_ref().unwrap().trim().is_empty() { continue; }
                ...
                match jira_client.get_issue(jira_key, &jira_fields).await {
                    Ok(existing_issue) => {
                            ...
                            if jira_ts > sheet_ts {
                                match params.conflict_resolution {
                                    ConflictResolution::SpreadsheetWins => {
                                        report.conflicts.push(ConflictReport {
                                            row: row_num, jira_key: jira_key.clone(),
                                            reason: format!("Jira newer ({}) vs Spreadsheet ({})", jira_ts, sheet_ts),
                                            resolution: "Spreadsheet wins".to_string(),
                                        });
                                    }
                                    ConflictResolution::JiraWins => {
                        ...
                        match jira_client.update_issue(jira_key, &field_update).await {
                            ...
                            Err(e) => { report.errors.push(SyncError { row: row_num, jira_key: Some(jira_key.clone()), error: format!("Update failed: {}", e) }); }
                        }
                    }
                    Err(e) => { report.errors.push(SyncError { row: row_num, jira_key: Some(jira_key.clone()), error: format!("Fetch failed: {}", e) }); }
                }
            } else {
                let field_update = build_field_update(&row);
                match jira_client.create_issue(&params.jira_project_key, &field_update).await {
                    Ok(new_key) => {
                        report.created += 1;
                        tracing::info!(row = row_num, jira_key = %new_key, "created Jira issue");
                    }
                    Err(e) => { report.errors.push(SyncError { row: row_num, jira_key: None, error: format!("Create failed: {}", e) }); }
                }
            }
        } else {
            let field_update = build_field_update(&row);
...
fn read_spreadsheet_rows(
    ...
    let sheet = book.get_sheet_by_name(sheet_name).ok_or_else(|| anyhow!("sheet '{}' not found", sheet_name))?;
    ...
    loop {
        ...
        if summary.is_none() || summary.as_ref().unwrap().trim().is_empty() { break; }
        let status = mapping.status_column.as_ref().and_then(|col| get_cell_value(sheet, col, row_num));
        let assignee = mapping.assignee_column.as_ref().and_then(|col| get_cell_value(sheet, col, row_num));
        let updated = mapping.updated_column.as_ref().and_then(|col| get_cell_value(sheet, col, row_num)).and_then(|s| parse_timestamp(&s));
        let description = mapping.description_column.as_ref().and_then(|col| get_cell_value(sheet, col, row_num));
        let priority = mapping.priority_column.as_ref().and_then(|col| get_cell_value(sheet, col, row_num));
        let labels = mapping.labels_column.as_ref().and_then(|col| get_cell_value(sheet, col, row_num)).map(|s| s.split(',').map(|l| l.trim().to_string()).collect());
        rows.push(SpreadsheetRow { jira_key, summary, status, assignee, updated, description, priority, labels });
        row_num += 1;
    }
    Ok(rows)
}
...
fn get_cell_value(sheet: &umya_spreadsheet::Worksheet, column: &str, row: usize) -> Option<String> {
    ...
    sheet.get_cell(address.as_str()).map(|cell| cell.get_value().to_string())
}

fn build_row_edits(sheet_name: &str, row_num: usize, mapping: &JiraSyncColumnMapping, issue: &JiraIssue) -> Vec<EditOp> {
    let mut edits = Vec::new();
    let now = Utc::now();
    edits.push(EditOp { timestamp: now, sheet: sheet_name.to_string(), address: format!("{}{}", mapping.jira_key_column, row_num), value: issue.key.clone(), is_formula: false });
    if let Some(summary) = issue.fields.get("summary").and_then(|v| v.as_str()) {
        edits.push(EditOp { timestamp: now, sheet: sheet_name.to_string(), address: format!("{}{}", mapping.summary_column, row_num), value: summary.to_string(), is_formula: false });
    }
    if let Some(ref col) = mapping.status_column {
        if let Some(status) = issue.fields.get("status").and_then(|v| v.get("name")).and_then(|v| v.as_str()) {
            edits.push(EditOp { timestamp: now, sheet: sheet_name.to_string(), address: format!("{}{}", col, row_num), value: status.to_string(), is_formula: false });
        }
    }
    if let Some(ref col) = mapping.assignee_column {
        if let Some(assignee) = issue.fields.get("assignee").and_then(|v| v.get("displayName")).and_then(|v| v.as_str()) {
            edits.push(EditOp { timestamp: now, sheet: sheet_name.to_string(), address: format!("{}{}", col, row_num), value: assignee.to_string(), is_formula: false });
        }
    }
    if let Some(ref col) = mapping.updated_column {
        if let Some(updated) = issue.fields.get("updated").and_then(|v| v.as_str()) {
            edits.push(EditOp { timestamp: now, sheet: sheet_name.to_string(), address: format!("{}{}", col, row_num), value: updated.to_string(), is_formula: false });
        }
    }
    if let Some(ref col) = mapping.description_column {
        if let Some(desc) = issue.fields.get("description").and_then(|v| v.as_str()) {
            edits.push(EditOp { timestamp: now, sheet: sheet_name.to_string(), address: format!("{}{}", col, row_num), value: desc.to_string(), is_formula: false });
        }
    }
    if let Some(ref col) = mapping.priority_column {
        if let Some(priority) = issue.fields.get("priority").and_then(|v| v.get("name")).and_then(|v| v.as_str()) {
            edits.push(EditOp { timestamp: now, sheet: sheet_name.to_string(), address: format!("{}{}", col, row_num), value: priority.to_string(), is_formula: false });
        }
    }
    if let Some(ref col) = mapping.labels_column {
        if let Some(labels) = issue.fields.get("labels").and_then(|v| v.as_array()) {
            let labels_str = labels.iter().filter_map(|l| l.as_str()).collect::<Vec<_>>().join(", ");
            edits.push(EditOp { timestamp: now, sheet: sheet_name.to_string(), address: format!("{}{}", col, row_num), value: labels_str, is_formula: false });
        }
    }
    edits
}
...
fn apply_edits_to_spreadsheet(path: &std::path::Path, sheet_name: &str, edits: &[EditOp]) -> Result<()> {
    let mut book = umya_spreadsheet::reader::xlsx::read(path).context("read spreadsheet failed")?;
    let sheet = book.get_sheet_by_name_mut(sheet_name).ok_or_else(|| anyhow!("sheet '{}' not found", sheet_name))?;
    for edit in edits {
        let cell = sheet.get_cell_mut(edit.address.as_str());
        if edit.is_formula { cell.set_formula(edit.value.clone()); } else { cell.set_value(edit.value.clone()); }
    }
...
fn parse_jira_timestamp(fields: &serde_json::Value, field_name: &str) -> Option<DateTime<Utc>> {
    fields.get(field_name).and_then(|v| v.as_str()).and_then(|s| DateTime::parse_from_rfc3339(s).ok()).map(|dt| dt.with_timezone(&Utc))
}

fn parse_timestamp(s: &str) -> Option<DateTime<Utc>> {
    DateTime::parse_from_rfc3339(s).ok().map(|dt| dt.with_timezone(&Utc)).or_else(|| {
        chrono::NaiveDateTime::parse_from_str(s, "%Y-%m-%d %H:%M:%S").ok().map(|ndt| DateTime::<Utc>::from_naive_utc_and_offset(ndt, Utc))
    })
}
...
mod tests {
    ...
    fn test_conflict_resolution_default() {
        assert!(matches!(ConflictResolution::default(), ConflictResolution::JiraWins));
    }
